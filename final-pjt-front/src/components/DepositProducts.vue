<template>
  <div>
    ...
    <div class="product-list">
      <div v-for="product in filteredDepositProducts" :key="product.fin_prdt_cd" class="product-card">
        <div class="card-info">
        ...
        <div class="button-container">
          <RouterLink :to="{ name: 'DepositProductDetailView', params: { fin_prdt_cd: product.fin_prdt_cd }}">
...
const filteredDepositProducts = computed(() => {
  if (props.selectedBank === '전체') {
    ...
    return store.depositProducts.filter(product => product.kor_co_nm === props.selectedBank);
  }
});
...
onMounted(async () => {
  ...
  await loadFavoriteDepositProducts();
});
...
const loadFavoriteDepositProducts = async () => {
  try {
    ...
    favoriteDepositProducts.value = response.data
      .filter(product => product.user === userId)
      .map(product => product.deposit_product);
    console.log('사용자의 관심 예금 상품 목록:', favoriteDepositProducts.value);
  } catch (error) {
...
const isFavorite = productId => {
  return favoriteDepositProducts.value.includes(productId);
};
...
const addToFavorites = async productId => {
  try {
    await axios.post(
      `${store.API_URL}/financial/favorite/deposit/add/`,
      { user: store.user.pk, deposit_product: productId },
      { headers: { Authorization: `Token ${store.token}` } }
    );
    favoriteDepositProducts.value.push(productId);
...
const removeFromFavorites = async productId => {
  try {
    await axios.delete(
      `${store.API_URL}/financial/favorite/deposit/remove/${productId}`,
      { headers: { Authorization: `Token ${store.token}` } }
    );
    favoriteDepositProducts.value = favoriteDepositProducts.value.filter(id => id !== productId);
