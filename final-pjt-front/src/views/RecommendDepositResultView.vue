  ...
  <div class="recommend-result">
    <div class="container">
      ...
      <h2 class="result" v-if="calculatedAmount">
        <span class="user-name">{{ store.name }}</span>님은 {{ store.depositPeriod }}개월 뒤에 
      ...
      <div v-if="topDepositProducts.length" class="top-products">
        <a
          ...
          :href="`/deposit-product-options/${product.product.fin_prdt_cd}`"
          target="_blank"
          class="product-card"
        >
      ...
      <h2 class="recommend-title mt-5 text-center">추천 예금 상품</h2>
      <div v-if="remainingDepositProducts.length">
        <ul class="product-list">
          <li v-for="product in remainingDepositProducts.slice(0, showAll ? remainingDepositProducts.length : 5)" :key="product.fin_prdt_cd">
            <RouterLink :to="`/deposit-product-options/${product.fin_prdt_cd}`" class="product-link">
...
const calculateDepositAmount = (amount, rate, period) => {
...
const fetchDepositProducts = async () => {
  loading.value = true;
  try {
    const response = await axios.get('http://localhost:8000/financial/filtered-deposit-products/', {
      params: {
        amount: store.depositAmount,
        period: store.depositPeriod,
      },
    });
    console.log('Filtered Deposit Products Response:', response.data);
    filteredDepositProducts.value = response.data.map(product => ({
      ...product,
    ...
    const topProductIds = filteredDepositProducts.value.map(product => product.id).slice(0, 3).join(',');
    const topOptionsResponse = await axios.get('http://localhost:8000/financial/top-deposit-products/', {
      params: { 'product_ids': topProductIds }
    });
    console.log('Top Deposit Products Response:', topOptionsResponse.data);
    topDepositProducts.value = topOptionsResponse.data;

    if (topDepositProducts.value.length > 0) {
      const bestOption = topDepositProducts.value[0].options[0];
      calculatedAmount.value = calculateDepositAmount(store.depositAmount, bestOption.intr_rate2, store.depositPeriod);
    }
  } catch (err) {
...
onMounted(() => {
  if (store.depositAmount === 0 || store.depositPeriod === 0) {
...
const remainingDepositProducts = computed(() => {
  const remaining = filteredDepositProducts.value.filter(product => !topDepositProducts.value.some(topProduct => topProduct.product.fin_prdt_cd === product.fin_prdt_cd));
  console.log('Remaining Deposit Products:', remaining);
  return remaining;
});
</script>
