...
onMounted(() => {
  store.savingMonthlyAmount = 0;
  store.savingPeriod = 0;
  amountInput.value.focus();
});

const nextStep = () => {
  store.savingMonthlyAmount = savingMonthlyAmount.value;
  store.savingPeriod = savingPeriod.value;
  router.push({ name: 'RecommendLoadingView' });
  setTimeout(() => {
    router.push({ name: 'RecommendSavingResultView' });
