<template>
  <div class="recommend-age">
    <div class="progress-bar">
      <div class="progress" :style="{ width: '28.5714285%' }"></div>
    </div>
    <h2>나이를 입력하세요</h2>
    <input v-model="age" type="number" placeholder="(만) 나이" ref="ageInput" @keyup.enter="nextStep" />
    <button class="button" @click="nextStep">다음</button>
  </div>
</template>
...
const nextStep = () => {
  store.age = age.value;
  router.push({ name: 'RecommendGenderView' });
};
