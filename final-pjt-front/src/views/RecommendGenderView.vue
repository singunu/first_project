<template>
  <div class="recommend-gender">
    <div class="progress-bar">
      <div class="progress" :style="{ width: '42.85714285714286%' }"></div>
    </div>
    <h2>성별을 선택하세요</h2>
    <div class="gender-buttons">
      <button :class="{'button': true, 'selected': gender === 'male'}" @click="selectGender('male')">남성</button>
      <button :class="{'button': true, 'selected': gender === 'female'}" @click="selectGender('female')">여성</button>
    </div>
  </div>
</template>
...
const selectGender = (selectedGender) => {
  ...
  router.push({ name: 'RecommendBusinessView' });
};
</script>
