<template>
  <div class="recommend-work">
    <div class="progress-bar">
      <div class="progress" :style="{ width: '71.42857142857143%' }"></div>
    </div>
    <h2>중소기업 재직 여부를 선택하세요</h2>
    <div class="work-buttons">
      <button :class="{'button': true, 'selected': isSMEWorker === true}" @click="selectWork(true)">예</button>
      <button :class="{'button': true, 'selected': isSMEWorker === false}" @click="selectWork(false)">아니요</button>
    </div>
  </div>
</template>
...
const isSMEWorker = ref(null);

const selectWork = (value) => {
  isSMEWorker.value = value;
  store.isSMEWorker = value;
  router.push({ name: 'RecommendDepositSavingView' });
};
</script>
